...
from common.errors import DetailInfoError, DataBaseError, AuthError, ParserError, Error502, ReloadSession
...
class AVBy:
    ...
    def __init__(self):
        ...
        self.max_page_search = 25    # Максимальное количество первых страниц, которые нужно мониторить (по 25 на стр.)
        if self.TEST:
        ...
        self.recaptcha_token_file_name = 'reCAPTCHA_token'

    @staticmethod
    def write_to_file(file_name, data: dict, mode='w+', indent=4) -> None:
    ...
    def read_from_json_file(file_name: str) -> dict:
        ...
        with open(f'{file_name}.json', 'r') as f:
            result_str = f.read()
            result = json.loads(result_str) or dict()
        return result
    ...
    async def set_cookie_in_aiosession_header(self, request_url: Optional[str] = None,) -> None:
        try:
            # print(f'cookies = ', self.aiosession.cookie_jar._cookies)
            raw_cookie_list = []
            for cookie in self.aiosession.cookie_jar:
                domain = cookie['domain']
                if request_url:
                    request_url = URL(request_url)
                    hostname = request_url.raw_host or ""
                    if domain == hostname:
                        raw_cookie_list.append(f'{cookie.key}={cookie.value}')
                else:
                    raw_cookie_list.append(f'{cookie.key}={cookie.value}')

            self.raw_cookies_str = '; '.join(raw_cookie_list)
            self.aiosession._default_headers['Cookie'] = self.raw_cookies_str
    ...
    def get_reCAPTCHA_token(self):
        try:
            api_key = '87a89024d6e9e46f6d30f78ff4591d85'
            site_key = '6LewiPMbAAAAAGivApIOmNe4pIjnoWgi5gjRdcW2'

            client = AnticaptchaClient(api_key)
            task = NoCaptchaTaskProxylessTask(self.login_url, site_key)
            job = client.createTask(task)
            self.logger.info('Waiting to solution by Anticaptcha workers')
            job.join()
            # Receive response
            g_recaptcha_response = job.get_solution_response()
            self.logger.debug(f'{g_recaptcha_response = }')
            self.logger.debug(f'Token Write to file {self.recaptcha_token_file_name}')
    ...
    async def get_auth(self):
        """Log in and get the necessary cookies."""

        try:
            self.user_login = 'artsemk@ya.ru'   # +375298644551
            # self.password = 'zEzcaj-zoqduc-hyndo6'
            self.password = 'asd123321'

            login_url = 'https://av.by/login'
            await self.fetch(login_url)

            sing_in_url = 'https://api.av.by/auth/login/sign-in'
            await self.fetch(sing_in_url, method='options')
            await self.set_cookie_in_aiosession_header()

            recaptcha_token = self.get_reCAPTCHA_token()
            data = {
                "googleRecaptcha2InvisibleToken": recaptcha_token,
                'login': self.user_login,
                'password': self.password,
            }

            self.aiosession._default_headers['Content-Type'] = 'application/json'
            self.aiosession._default_headers['x-device-type'] = 'web.desktop'
            resp = await self.fetch(sing_in_url, method='post', json=data)

            page = unescape(resp.decode('windows-1251'))
            resp_data = json.loads(page)
            # print(json.dumps(resp_data, indent=4, ensure_ascii=False))
            apiKey = resp_data['apiKey']
            userGroup = resp_data['userGroup']
            oldSessionId = resp_data['oldSessionId']
            oldRememberMeHash = resp_data['oldRememberMeHash']
            oldRememberMeId = resp_data['oldRememberMeId']

            params = {
                'force': 'false'
            }
            state_url = 'https://api.av.by/state?force=false'
            await self.fetch(state_url, method='options', params=params)
            await self.set_cookie_in_aiosession_header()

            self.aiosession._default_headers['x-api-key'] = apiKey
            self.aiosession._default_headers['x-user-group'] = userGroup
            await self.fetch(state_url, params=params)
            await self.set_cookie_in_aiosession_header()
            raw_cookie_list = [
                f'userGroup={userGroup}',
                f'PHPSESSID={oldSessionId}',
                f'avby_hash={oldRememberMeHash}',
                f'avby_id={oldRememberMeId}',
                f'X-Api-Key={apiKey}',
            ]
            self.raw_cookies_str = '; '.join(raw_cookie_list)
            self.aiosession._default_headers['Cookie'] = self.raw_cookies_str
            ...
            page = resp.decode('windows-1251')
            tree = document_fromstring(page)
            # user_name = tree.find('.//h4[@class="nav__dropdown-header"]/span').text_content().strip()
            user_name = tree.find('.//h4[@class="nav__dropdown-name"]/span').text_content().strip()

            if user_name != 'Артем':
                raise AuthError("Can't find user_name in main page.")

            self.logger.debug(f"[auth] user_name = {user_name}")

            list_cookies = [
                {'userGroup': userGroup},
                {'PHPSESSID': oldSessionId},
                {'avby_hash': oldRememberMeHash},
                {'avby_id': oldRememberMeId},
                {'X-Api-Key': apiKey},
            ]
            for cookies in list_cookies:
                self.aiosession.cookie_jar.update_cookies(cookies, URL(self.main_url))

            # TODO: Допилить, чтоб куки читались из файла. Вдруг сработает
            # print(f'cookies = ', self.aiosession.cookie_jar._cookies)
            # self.write_to_file(file_name=self.initial_cookies_filename, data=self.initial_cookies_list_dict)
        ...
        try:
            ...
            async with aiohttp.ClientSession(connector=conn, headers=headers) as self.aiosession:
                ...
                while True:
                    ...
                    # TODO: Включить проверку ftp соединения
