...
class SendAlreadySentInactiveCarsAvBy:
    ...
    async def sent_post_request(self, cars_id: int, dict_data: dict) -> None:
        ...
        try:
            ...
            self.update_list.append((sent_at, is_send, cars_id))
    ...
    async def send_already_sent_inactive_cars(self):
        """Отправляем не активные авто, которые ещё не отправляли."""
        try:
            already_sent_inactive_cars_tuple = await db.select_already_sent_inactive_cars(table_name=self.table_name,
                                                                              dependent_table=self.dependent_table,
                                                                              limit=self.limit)
            while already_sent_inactive_cars_tuple:
                self.list_dict_data_for_sending = []
                self.update_list = []
                self.count_send = 0

                for tuple_data in already_sent_inactive_cars_tuple:
                ...
                if self.update_list:
                    # update_list = [(sent_at, is_send, cars_id), (sent_at, is_send, cars_id), ...]
                    await db.mark_already_sent_inactive_car_as_sent(dependent_table=self.dependent_table,
                                                       list_values=self.update_list)
                else:
                    raise ParserError('No self.update_list !!!')

                self.logger.info(f'[send_inactive_cars] last_db_id = {already_sent_inactive_cars_tuple[-1][0]}')
                self.logger.debug(f'[send_inactive_cars] self.count_send = {self.count_send}')

                already_sent_inactive_cars_tuple = await db.select_already_sent_inactive_cars(
    ...
    async def run(self):
        ...
        try:
            ...
            async with aiohttp.ClientSession(connector=conn, headers=headers) as self.aiosession:
                ...
                await self.send_already_sent_inactive_cars()
